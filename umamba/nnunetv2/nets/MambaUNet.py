...
class MambaMLP(nn.Module):
    def __init__(self, layer_num, hidden_size, d_state=64, d_conv=4, expand=4,mha_head=8):
        super(MambaMLP, self).__init__()
        encoders = [
            MambaMLPLayer(hidden_size, d_state, d_conv, expand, mha_head) for _ in range(layer_num)
        ]
...
class MambaUNet(nn.Module):
    def __init__(self,
        ...
        self.mamba_mlp = MambaMLP(12, features_per_stage[-1], d_state=64, d_conv=4, expand=4)
    ...
    def forward(self, x):
        ...
        middle_feature_flat = self.mamba_mlp(middle_feature_flat)
        out = middle_feature_flat.transpose(-1, -2).view(B, C, *img_dims)
        skips[-1] = out
